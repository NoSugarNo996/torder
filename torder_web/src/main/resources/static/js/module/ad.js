            ...
            success: function (res) {
                ...
                for (let index = 0; index < res.data.length; index++) {
                    data += '<tr>\n' +
                        ...
                        '<td>\n' +
                        '<div class="operate">\n' +
                        '<button type="button" class="checkDetail layui-btn layui-btn-sm layui-btn-primary" value="' + res.data[index].adId + '">编辑</button>\n' +
                        '<button type="button" class="endorseEvent layui-btn layui-btn-sm layui-btn-warm" value="' + res.data[index].adId + '">禁用</button>\n' +
                        '</div>\n' +
                        '</td>\n' +
                        '</tr>\n'
                }
...
//删除
    $('body').on('click', '.endorseEvent', function () {
        $.ajax({
            type: "get",
            url: queryUrl + queryMethodAdvertising + deleteMethod,
            data: {
                "id": $(this).attr('value')
            },
            success: function (res) {
